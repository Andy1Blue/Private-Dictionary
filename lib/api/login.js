...
login.post = (req, res) => {
  ...
  const password = typeof(req.headers.password) == 'string' && req.headers.password.trim().length > 5 ? req.headers.password.trim() : false;
  if(email && password) {
    console.log(email, password)
    db.read('dictionary_words_user', 'email', email)
      .then(dataUser => {
